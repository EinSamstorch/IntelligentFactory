package cloud.behaviours.simple;
...
/**
 * SimpleBehaviour类,用于将订单状态推送至云端与保存到本地mysql中.
 * 失败重试最大次数:3，失败后将写入日志.
 *
...
public class Push2CloudBehaviour extends SimpleBehaviour {
    ...
    /**
     * 更新工件位置信息.
     * API网址：http://{website}/ks/FactoryAction_updateJobMachine.action? .
     */
    ...
    */
    ...
    public Push2CloudBehaviour(CloudAgent ca, String param, int choice) {
        ...
        switch (choice) {
            case UPDATE_STATE:
                url = String.format("http://%s/ks/FactoryAction_updateJobState.action?", cagent.getWebsite());
                break;
            case UPDATE_POSITION:
                url = String.format("http://%s/ks/FactoryAction_updateJobMachine.action?", cagent.getWebsite());
                break;
        }
    }
...
//        CloudMysql sqlTool = new CloudMysql(cagent.getSetting());
//        switch (choice){
//
//        }
        // 未来再说
