...
public class DfUtils {
  ...
  /**
   * 创建 招标消息 消息类别 CFP 消息条约 合同网 超时时间 10秒.
   *
   * @param content 消息载体 object类型 需实现Serializable接口
   * @return 招标消息
   */
  public static ACLMessage createCfpMsg(Object content) throws MessageCreateFailedException {
  ...
  /**
   * 创建 请求消息 消息类别 REQUEST 消息条约 REQUEST.
   *
   * @param content 消息载体 object类型 需实现Serializable接口
   */
  public static ACLMessage createRequestMsg(Object content) throws MessageCreateFailedException {
    ACLMessage msg = createRequestMsgHeader();
    msgSetContent(msg, content);
    return msg;
  }
